...
掃表是一款 Android QR Code 表單自動填寫工具。掃描 Google 表單或其他 HTTPS 問卷後，App 會在內建的安全 WebView 中讀取題目，依基本資料、自訂規則或預設回覆填入欄位，最後停在提交前讓使用者檢查。
...
1. 開啟 App 並允許相機權限。
2. 點首頁右上角「我的資料」，可選填姓名、Gmail／Email、大學、科系、年級與學號，修改簡答／長答的預設文字回覆，或新增最多 12 組「題目關鍵詞 → 回覆」。
...
- 簡答、長答、可編輯文字區、Email、電話、數字與網址
- 單選、多選與常見量表
- 原生下拉選單與 ARIA listbox
- 日期、時間、月份與 datetime-local
- Google Forms，以及同頁混用多種標準 HTML／ARIA 題目容器的常見 HTTPS 表單

檔案上傳、CAPTCHA、登入驗證，以及用 Canvas 或完全自訂元件製作的題型可能需要手動處理。表單網站改版後，選擇器也可能需要跟著更新。

## 回答規則

App 使用本機規則填寫，不會把題目送到 AI 服務或其他伺服器。未命中基本資料與自訂規則的簡答／長答預設為「無」；Email、電話、數字、網址與日期等有格式要求的欄位仍使用可通過格式檢查的值。單選與多選會選擇可用選項，數字量表預設選擇中間值。
...
「預設文字回覆」可在我的資料頁自行修改，會套用到未命中資料或自訂規則的一般簡答與長答題。未設定時使用「無」；舊版未修改過的預設長答會自動轉為「無」，自行寫入的回答仍保留。
...
- 無法辨識或未填妥的題目會在表單頁顯示待手動處理數量；檔案上傳等題型仍需自行完成。
- 不儲存掃描紀錄或表單答案。
- 不會自動提交表單。
