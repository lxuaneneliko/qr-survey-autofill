...
1. 開啟 App 並允許相機權限。
2. 點首頁右上角「我的資料」，可選填姓名、Gmail／Email、大學、科系與年級。
3. 用相機掃描 QR Code、從圖庫選擇 QR Code 圖片，或在掃描框下方貼上 HTTPS 表單連結。
4. App 內建表單頁載入題目並自動填寫；題意明確相符時優先套用基本資料，並在頂端列出實際套用項目。
5. 使用者檢查答案並自行按下提交。
...
使用者儲存基本資料後，App 會比對題目標題、欄位標籤、提示文字與選項。只有高信心匹配才會套用；例如「姓名／全名」對應姓名、「Gmail／電子信箱」對應 Email、「大學／院校」對應學校。第三方聯絡人、公司名稱、GPA、學籍狀態等容易混淆的欄位會跳過。個人資料選單找不到相符選項時會保持未選，不會退回亂選第一項。
...
- 手動貼上的連結只用來開啟表單，不會儲存。
- 基本資料只存在 App 私有的本機儲存空間，不納入 Android 備份；不會要求或儲存 Gmail 密碼。
