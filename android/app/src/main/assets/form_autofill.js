(async function () {
  ...
  function generatedValue(element, root) {
    const context = contextText(element, root);
    const type = String(element.getAttribute("type") || "text").toLowerCase();
    if (type === "date") return `${yyyy}-${mm}-${dd}`;
    if (type === "time") return "10:00";
    if (type === "datetime-local") return `${yyyy}-${mm}-${dd}T10:00`;
    if (type === "month") return `${yyyy}-${mm}`;
    if (type === "week") return `${yyyy}-W01`;
    if (context.includes("年") && (context.includes("日期") || context.includes("生日"))) return yyyy;
    if (context.includes("月") && (context.includes("日期") || context.includes("生日"))) return String(now.getMonth() + 1);
    if (context.includes("日") && (context.includes("日期") || context.includes("生日"))) return String(now.getDate());
    if (type === "email" || /email|e-mail|電子郵件|信箱|郵件/.test(context)) return "demo@example.com";
    if (type === "tel" || /電話|手機|聯絡號碼|phone|mobile/.test(context)) return "0912345678";
    if (type === "url" || /網址|網站|url|website/.test(context)) return "https://example.com";
    if (type === "number" || /年齡|歲數|數量|人數|age|number/.test(context)) return "20";
    if (/姓名|名字|稱呼|name/.test(context)) return "測試使用者";
    if (/學校|單位|公司|organization|company|school/.test(context)) return "測試單位";
    if (/意見|建議|原因|心得|說明|描述|回饋|留言|comment|feedback|description|why/.test(context) || element.tagName === "TEXTAREA") {
      return "這是由掃表自動產生並填入的回覆。";
    }
    return "自動填寫";
  }
  ...
  function chooseOne(elements) {
    const choices = elements.filter(visible);
    if (!choices.length || choices.some(isSelected)) return false;
    ...
    return safeClick(choices[index]);
  }

  async function fillListbox(listbox) {
    const selectedOption = listbox.querySelector('[role="option"][aria-selected="true"]');
    const current = normalize(selectedOption?.textContent || selectedOption?.getAttribute("aria-label"));
    if (current && !/^(選擇|請選擇|choose|select|dropdown|下拉式選單)$/i.test(current)) return false;
    if (!safeClick(listbox)) return false;
    await wait(120);
    ...
    return option ? safeClick(option) : false;
  }
  ...
  async function fillRoot(root) {
    ...
    for (const input of textInputs) setValue(input, generatedValue(input, root));
    ...
    if (roleRadioGroups.length) {
      for (const group of roleRadioGroups) chooseOne(Array.from(group.querySelectorAll('[role="radio"]')));
    } else {
      chooseOne(Array.from(root.querySelectorAll('[role="radio"]')));
    }
    ...
    for (const name of radioNames) {
      chooseOne(nativeRadios.filter((radio) => (radio.name || "__unnamed") === name));
    }
    ...
    for (const listbox of listboxes) await fillListbox(listbox);

    return { answered: rootAnswered(root), unsupported: fileInputs.length > 0 };
  }
  ...
  try {
    ...
    if (window.QRSurveyNative && typeof window.QRSurveyNative.report === "function") {
      window.QRSurveyNative.report(JSON.stringify({ filled, total: roots.length, unsupported }));
    }
  } catch (error) {
    if (window.QRSurveyNative && typeof window.QRSurveyNative.report === "function") {
      window.QRSurveyNative.report(JSON.stringify({ filled: 0, total: 0, unsupported: 0, error: String(error) }));
    }
  } finally {
